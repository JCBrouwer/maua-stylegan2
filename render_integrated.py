...
import nvsg2a.dnnlib, nvsg2a.legacy
...
if __name__ == "__main__":
    ...
    a_lil_noise = 0.5 * th.randn(size=(1, 1, 4, 8), device="cuda")
    ...
    for name, param in G.synthesis.named_parameters():
        if "noise_strength" in name:
            ...
            if h > 4:
                setattr(net, "noise_const", th.randn(size=(1, 1, h, 2 * w), device="cuda"))
    ...
    def generate_input(queue_out):
        window = gaussian_kernel(5)[:, None]
        z = th.randn(size=(512,), device="cuda")
        noise_buffer = th.randn(size=(len(window), noise_numel), device="cuda")
        for _ in range(num_frames):
            z = slerp(0.05, z, th.randn(size=(512,), device="cuda"))
            latent = G.mapping(z[None, :], c=None)

            noise_buffer = th.roll(noise_buffer, shifts=1, dims=0)
            noise_buffer[-1] = th.randn(noise_numel, device="cuda")
            noise = th.sum(window * noise_buffer, axis=0)

            queue_out.put((latent, noise))

    inference_queue = queue.Queue()
    generate_thread = Thread(target=generate_input, args=(inference_queue,))
